...
**Always preview and screenshot over HTTP — start a local web server first and load the HTML via its `http://localhost:…` URL; never open the file directly (`file://`).** This is required for multi-file prototypes (their `<script type="text/babel" src="…jsx">` components load only over HTTP, so `file://` silently fails) and is the standard for self-contained single files too, so previews and screenshots stay consistent. Serve a single `designs` server for the whole `designs/` directory (`python3 -m http.server 4311 --directory designs`) so every project shares one server, open `http://localhost:4311/<project>/<file>.html`, check the console for JS errors, and screenshot to inspect layout. Fix any errors and surface it again. The exact preview / console / screenshot tools — and the preview-harness gotchas (React `onClick` delegation, keyboard-event dispatch, screenshot desync) plus the MCP-unavailable `file://` fallback for self-contained files — are in your reference doc (`references/claude.md` or `references/cursor.md`).

For thorough or directed checks ("screenshot and check the spacing"), spawn a verification subagent (an `Agent` on Claude Code, a `browser-use` `Task` on Cursor — see your reference doc) to load the file, take screenshots, probe the JS, and report back — useful when you don't want to clutter your own context.
