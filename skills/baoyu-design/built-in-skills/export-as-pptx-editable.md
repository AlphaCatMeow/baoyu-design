...
If the deck uses the `data-anim` convention ([make-a-deck](make-a-deck.md) → *Animations*), gen_pptx detects the attributes automatically — there is no input field to set — and writes them as native PowerPoint entrance/exit/emphasis/motion-path animations with the authored trigger, delay, duration, and order. Capture is unaffected: slides are still captured in their base (finished) layout. The result JSON includes an `animations` count so you can confirm the builds made it into the file.
...
- `animation_invalid` — a `data-anim-*` value had a problem; read the message for which of two outcomes applies. Recoverable values (bad trigger/dir/delay/duration/order, over-long paths) fall back to defaults and the animation **is** still exported; unusable ones (unknown effect, `rotate` 0, `scale` 1, missing/bad path, `data-anim` on the slide root, element hidden at capture) drop the animation and the element exports statically. Fix the attribute (see [make-a-deck](make-a-deck.md) → *Animations*) and re-export.
